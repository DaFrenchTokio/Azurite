...
function __UI__:Notification(a)pcall(function()game.StarterGui:SetCore("SendNotification",{Title="Azurite",Text=tostring(a),Icon="rbxthumb://type=Asset&id=10912483183&w=150&h=150",Duration=5})end)end
...
function __UI__:Page()
	...
	local Main = {
		...
		SaveData = Instance.new("ImageButton"),
	...
	function __UI__.AddElementToLoad(element)
		...
		if (writefile and readfile) then
			if ElementsHolder.IsThereFile == false and ElementsHolder.Import == false then
	...
	if (readfile and isfile) then 
		if isfile(__UI__["#File"]) then
			...
			ElementsHolder.Import = true
		else
			__UI__._Debug(tostring(__UI__["#File"])..' | isfile [FALSE]') 
			ElementsHolder.IsThereFile = false
			ElementsHolder.Import = false
		end
	end

	__UI__._Debug('AZURITE | Library | Loading #File Status | ' .. tostring(ElementsHolder.Import))
	__UI__._Debug('----------------------------------------------------------------------------------')
	...
	Main.SaveData.Name = __UI__.RandomName("SaveData")
	Main.SaveData.Parent = Main.SideButtonsHolder
	Main.SaveData.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	Main.SaveData.BackgroundTransparency = 1.000
	Main.SaveData.Position = UDim2.new(0.5, 0, 0, 0)
	Main.SaveData.Size = UDim2.new(0, 17, 0, 17)
	Main.SaveData.Image = "http://www.roblox.com/asset/?id=6031289449"
	Main.SaveData.MouseButton1Click:Connect(function()
	...
	function Main:Category(CategoryTitle)
		if Counts.Category <= 7 then
			...
			function em:Toggle(__Title__, __Value__, __Call__)
				...
				if _xET.Value == true then
					CallToggle.Position = UDim2.new(0.538, 0,0.063, 0)
				elseif _xET.Value == false then
					CallToggle.Position = UDim2.new(0.005, 0,0.063, 0)
				else
					__UI__._Debug('ERROR | A Toggle has a value that is not excepted : ' ..tostring(_xET.Value))
					CallToggle.Position = UDim2.new(0.005, 0,0.063, 0)
				end
				CallToggle.Size = UDim2.new(0, 18, 0, 18)
				CallToggle.ZIndex = 3
